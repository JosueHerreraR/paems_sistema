<?php
	...
	if (!empty($_FILES['file_name']['name'])) {
		...
		$destination1 = 'fotos_cultural/'.$newFile1;
		$destination2 = 'fotos_cultural/'.$newFile2;
		$destination3 = 'fotos_cultural/'.$newFile3;
		...
		if (in_array($fileActExt, $allowImg)) {
			if ($_FILES['file_name']['size'] > 0 && $_FILES['file_name']['error']==0) {
				...
				$filePath1 = 'fotos_cultural/'.$row['cultural_1'];
                $filePath2 = 'fotos_cultural/'.$row['cultural_2'];
                $filePath3 = 'fotos_cultural/'.$row['cultural_3'];
				
				if (move_uploaded_file($fileTmp, $destination)) {
					$update = "UPDATE imagenes_escuelas SET cultural_1 = '$newFile1', cultural_2 = '$newFile2', cultural_3 = '$newFile3',WHERE id_img = '$image_id'";
