<?php
	...
	if (!empty($_FILES['file_name']['name'])) {
		...
		$destination1 = 'fotos_instalaciones/'.$newFile1;
		$destination2 = 'fotos_instalaciones/'.$newFile2;
		$destination3 = 'fotos_instalaciones/'.$newFile3;
		...
		if (in_array($fileActExt, $allowImg)) {
			if ($_FILES['file_name']['size'] > 0 && $_FILES['file_name']['error']==0) {
				...
				$filePath = 'fotos_instalaciones/'.$row['images'];
				
				if (move_uploaded_file($fileTmp, $destination)) {
					$update = "UPDATE imagenes_escuelas SET instalacion_1 = '$newFile1', instalacion_2 = '$newFile2', instalacion_3 = '$newFile3',WHERE id_img = '$image_id'";
