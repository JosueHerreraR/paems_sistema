<?php
	...
	if (!empty($_FILES['file_name']['name'])) {
		...
		$destination1 = 'fotos_pabellon/'.$newFile1;
		$destination2 = 'fotos_pabellon/'.$newFile2;
		$destination3 = 'fotos_pabellon/'.$newFile3;
		...
		if (in_array($fileActExt, $allowImg)) {
			if ($_FILES['file_name']['size'] > 0 && $_FILES['file_name']['error']==0) {
				...
				$filePath1 = 'fotos_pabellon/'.$row['pabellon_1'];
                $filePath2 = 'fotos_pabellon/'.$row['pabellon_2'];
                $filePath3 = 'fotos_pabellon/'.$row['pabellon_3'];
				
				if (move_uploaded_file($fileTmp, $destination)) {
					$update = "UPDATE imagenes_escuelas SET pabellon_1 = '$newFile1', pabellon_2 = '$newFile2', pabellon_3 = '$newFile3' WHERE id_img = '$image_id'";
